  ...
  OwnableUpgradeable
{
  ...
  function payForGas(
    ...
    uint256 _requiredPayment = quoteGasPayment(_destinationDomain, _gasLimit);
    require(
  ...
  function quoteGasPayment(
    ...
    return
      (_destinationGasCost * _tokenExchangeRate) / TOKEN_EXCHANGE_RATE_SCALE;
  }
  ...
  function getExchangeRateAndGasPrice(
    uint32 _destinationDomain
  ) public view override returns (uint128 tokenExchangeRate, uint128 gasPrice) {
    IGasOracle _gasOracle = destinationGasConfigs[_destinationDomain].gasOracle;
